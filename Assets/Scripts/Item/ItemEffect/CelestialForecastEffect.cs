...
public class CelestialForecastEffect : IItemEffect
{
    ...
    {
        _mediator = mediator;
        _mediator.DirectionSelector.OnEnemyDirectionChanged += HandleDirectionChanged; // 方向が変わった時にUIを切り替えるメソッドを呼び出す
    }
    ...
    private void HandleDirectionChanged(DirectionEnum direction)
    {
        ...
        _mediator.VisualController.ForecastDirectionButton(_effectDirection);
    }
    ...
    {
        _mediator.VisualController.ReleaseForecastDirectionButton(_effectDirection); // UIリセット
        _mediator.DirectionSelector.OnEnemyDirectionChanged -= HandleDirectionChanged; // 効果終了タイミングで不要になるため購読解除
    }
